...
const Form = ({ spot, type }) => {
  ...
  // Breaks on first render
  const [previewImg, setPreviewImg] = useState(type === "UpdateSpotForm" ? spot.SpotImages[0].url : "");
  const [imageUrl1, setImageUrl1] = useState(type === "UpdateSpotForm" ? spot.SpotImages[1].url : "");
  const [imageUrl2, setImageUrl2] = useState(type === "UpdateSpotForm" ? spot.SpotImages[2].url : "");
  const [imageUrl3, setImageUrl3] = useState(type === "UpdateSpotForm" ? spot.SpotImages[3].url : "");
  const [imageUrl4, setImageUrl4] = useState(type === "UpdateSpotForm" ? spot.SpotImages[4].url : "");
  ...
  if(type === "UpdateSpotForm" && !spot.SpotImages) return;
  ...
  const handleClick = async (e) => {
    ...
    if (errorArr.length > 0) {
      ...
      if (type === "CreateSpotForm") {
        ...
        const oldImageUrls = [spot.SpotImages[0], spot.SpotImages[1], spot.SpotImages[2], spot.SpotImages[3], spot.SpotImages[4]]
        const updatedSpot = await dispatch(thunkUpdateSpot(data, spot.id, imageArr, oldImageUrls));
        history.push(`/spots/${updatedSpot.id}`);
        // const updatedSpot = await dispatch(thunkUpdateSpot(data)).then(()=>{
        // })
      }
